...
;;this is a computerized version of a serious game
;;aiming at simulaing biomass fluxes at household and
;;village scale
;;2020-2021
__includes ["biomtransfer.nls" "market.nls" "message.nls" "exportresults.nls" "explo_core.nls" "market_explo.nls" "biomtransfer_explo.nls" "cropSy.nls"]
...
patches-own [
  ...
  manuregain
  crop1;;crops grown 1=sorghum, 2=cowpea, 3=mungbean, 4=sesame
  crop2
  ...
  manuretrack;;track manure application for bonuses.
]
...
to update
  ...
  hubnet-send pseudo "on farm income" item 0[onfarm_inc] of farmers with [player = idplays]
...
to set-up
  ...
  set season (list "Good" "Bad" "Bad" "Good" "Good");;fixed as played
  set mois (list "July" "November" "April")
...
to nextmonth
  ...
  ;;animals not fed die
  ask turtles with [(shape = "cow" or shape = "donkey" or shape = "sheep") and hunger > 0][
    die
  ]
  ...
  if (ticks mod 3) = 0 [
    ...
    if year = 3 [
      ask farmers [
        let ply player
        ;;;2 cows = 1 newborn
        if count out-link-neighbors with [shape = "cow" and canmove ="yes"] >= 2[
          let nwbrn floor (count out-link-neighbors with [shape = "cow" and canmove ="yes"] / 2)
        ...
        ;;;2 srum = 1 newborn
        if count out-link-neighbors with [shape = "sheep" and canmove ="yes"] >= 2[
          let nwbrn floor (count out-link-neighbors with [shape = "sheep" and canmove ="yes"] / 2)
        ...
        ;;;2 donkey = 1 newborn
        if count out-link-neighbors with [shape = "wolf" and canmove ="yes"] >= 2[
          let nwbrn floor (count out-link-neighbors with [shape = "wolf" and canmove ="yes"] / 2)
    ...
    ;update food insecurity
    ask farmers [
      ;;set onfarm_inc onfarm_inc - (food_unsecure * 2)
      set food_unsecure 0
    ]

    ;; residue left on field not grazed are turned into mulch
    ask turtles with [shape = "star" and hidden? != true][set mulch? true]
    ;ask turtles with [shape = "cylinder" and [pcolor] of patch-here != white][
    ; set age age + 1
    ;  if age > 2 [die]
    ;]
    ...
    ask farmers [
      ...
      set nmanure floor (count out-link-neighbors with [shape = "cow" and canmove = "yes"] * 1 / 2 )+
      floor (count out-link-neighbors with [shape = "sheep" and canmove = "yes"] / 8) +
      floor (count out-link-neighbors with [shape = "wolf" and canmove = "yes"] / 4);1cow=1manure/year
      ask patches with [plabel = ""][set plabel "99"]
      hatch nmanure [
        set shape "triangle"
        set color 36
        set typo "manure"
        move-to one-of patches with [plabel = xy and cultiv = "yes"]
        set heading random 361
        set size .75
      ]
      ask patches with [plabel = "99"][set plabel ""]

      ;;1 additional poultry if npoultry >= 2
      ;;>=3 because of the fictive bird on the board
      if count out-link-neighbors with [shape = "bird"] >= 3 [
        ;ask joueurs with [idplay = ply][
        ;  hubnet-send pseudo "warning" "You have newborn in your herd"
        ;]
        ask one-of out-link-neighbors with [shape = "bird"][hatch 1]
      ]
    ]
    ...
    ask turtles with [shape = "cow" or shape = "donkey" or shape = "sheep"]
    [set repro "no" set grazed "" set hunger 0 set energy 0]
    ;set livfeed? false
...
to resdistrib [typology ferme]
  ...
  if typology = "MOD" [
    ...
    ask turtles with [shape = "person farmer" and farm = ferme][
      set family_size 6
      set nplot 6
      set ncow 2;one-of (range 1 3 1)
      set ndonkey 1;one-of (range 1 2 1)
      set nsrum 3;one-of (range 2 4 1)
      set npoultry 2;one-of (range 1 6 1)
  ...
  ifelse month = "July" [
    ...
    foreach farmi [
      ask turtles with [farm = item n farmi and shape = "person farmer"][
        ...
        let posi pos
        ask patches with [plabel = ""][set plabel "99"]
        set fin patches with [cultiv = "yes" and (read-from-string plabel) = posi];;cultivate the same plot each year
        ask patch-here[
          ;;seed
          sprout nseed [
            set typo "seed2"
...
to livsim [gamer animal]
  ...
  ask cow with [grazed != "yes"][
    if any? turtles-here with[shape = "box"] [
      ifelse (animal = "cow" or animal = "wolf") [
        ...
        set energy energy + 0 set grazed "yes" set hunger 0
      ]
  ...
  ask farmers with [player = gamer][
    ask out-link-neighbors with [shape = "cow" and canmove = "no"][
      set grazed "yes" set hunger 0
    ]
  ]

  ;directfeed gamer
  foreignresidue gamer
  if month != "July" and count turtles with [shape = "cylinder" and hidden? = false] = 0 [
    repeat 1 [
  ...
  if month = "July" [ask cow with [grazed = "yes"] [set grazed ""]]
end
  ...
  ask cow with [grazed != "yes"][
    ifelse animal = "cow" [
      if any? turtles-here with[shape = "star" and (farm = farmlab or foreignaccess = "yes") and hidden? = false and mulch? != true and color != blue and color != magenta] [
        ...
        if en > 4 [set en 4]
        ask n-of en turtles-here with[shape = "star" and (farm = farmlab or foreignaccess = "yes") and hidden? = false and mulch? != true and color != blue and color != magenta] [
          die
        ]
    ...
    [
      if any? turtles-here with[shape = "star" and (farm = farmlab or foreignaccess = "yes") and hidden? = false and mulch? != true and color != blue and color != magenta] [
        let en count turtles-here with[shape = "star" and (farm = farmlab or foreignaccess = "yes") and hidden? = false and mulch? != true and color != blue and color != magenta]
        if animal = "wolf" and en > 3 [set en 3]
        if animal = "sheep" and en > 2 [set en 2]
        ask n-of en turtles-here with[shape = "star" and (farm = farmlab or foreignaccess = "yes") and hidden? = false and mulch? != true and color != blue and color != magenta] [
          die
        ]
        set energy energy + en
...
to livupdate [gamer]
  ...
  ask farmers with [player = gamer][
    ...
    set ntricycle count out-link-neighbors with[shape = "truck"] - ms
...
to directfeed
  ...
  let fac 1; 2 for cow 1 for donkey 1 for small ruminants
  let eff 1; 2 for conc 1 for residue

  if animal = "cattle" [set fac .5]
...
to concfeed [gamer]
  ...
  if app = "yes" [
    ...
    ask farmers with [player = gamer][
      let cow out-link-neighbors with [shape = "cow" and canmove = "yes" and grazed != "yes"]
      set forage (count out-link-neighbors with [typo = "conc" and
        [pcolor] of patch-here = white] - 1) * 2 ;;concentrate = double effect of residue
      let fourrage forage
      let rest 0
      ...
      ifelse count cow > 0 [
        ...
        let srum out-link-neighbors with [shape = "sheep" and canmove = "yes"  and grazed != "yes"]
...
to getmanure [gamer]
  ...
  let fam item 0 [farm] of farmers with [player = gamer]
  ask patches with [plabel = ""][set plabel "99"]

  if month = "July" and year > 1 [
    ask patches with [pcolor = rgb 0 255 0 and read-from-string plabel = pst][
      let chargeanim sum animhere
      set manuregain floor (chargeanim)
      ;set animhere manuregain
      sprout manuregain [
        set typo "manure"
        set shape "triangle"
        set hidden? false
    ...
    ask farmers with [player = gamer]
    [set manuregains sum [sum animhere] of patches with [pcolor = rgb 0 255 0 and read-from-string plabel = pst]
    ]
...
to reproduce [gamer animal]
  ...
  let myfarm item 0 [farm] of farmers with [player = gamer]

  if month = "April"[
    ask farmers with [player = gamer] [
      ask out-link-neighbors with [grazed != "yes" and canmove ="yes" and
        (shape = "cow" or shape = "sheep" or shape = "wolf")][
        set hunger hunger + 1
      ]
    ]
  ]
  if month = "June" [
    ask turtles with [farm = myfarm and hunger > 0][
      ;ask joueurs with [idplay = gamer][
      ;  hubnet-send pseudo "warning" "You lost animal(s) in your herd"
      ;  show (word pseudo " You lost animal(s) in your herd" )
      ;]
      die
    ]
  ]
  ;;reproduction. only cattle, donkeys and small ruminants are concerned
  ;;happens once, in year 3
  if year = 3 and month = "July"[
    ask farmers with [player = gamer][
      let nbov count out-link-neighbors with [shape = "cow" and canmove = "yes"]
      ask n-of (floor (nbov / 2)) out-link-neighbors with [shape = "cow" and canmove = "yes"][
        hatch 1 [set hunger 0]
      ]

      let ndky count out-link-neighbors with [shape = "wolf" and canmove = "yes"]
      ask n-of (floor (ndky / 2))out-link-neighbors with [shape = "wolf" and canmove = "yes"][
        hatch 1 [set hunger 0]
      ]

      let nprum count out-link-neighbors with [shape = "sheep" and canmove = "yes"]
      ask n-of (floor (nprum / 2))out-link-neighbors with [shape = "sheep" and canmove = "yes"][
        hatch 1 [set hunger 0]
      ]
    ]
  ]
...
to feedfamily [gamer]
  ...
  ask farmers with [player = gamer][
    ...
    [set food_unsecure 0 set ngrain floor (ngrain - nfeed)
      ask n-of ceiling nfeed out-link-neighbors with [shape = "cylinder"][die]
    ]
    set feedfam 1
  ]
end
...
"player 3" 1.0 0 -14439633 true "" "if ticks > 0 [\nplot item 0 [ncow] of turtles\nwith [farm = \"3\" and shape = \"person farmer\"]\n]"
...
"" ""
PENS
"player 1" 1.0 0 -13791810 true "" "if ticks > 0 [\nplot item 0[onfarm_inc] of turtles with [\nshape = \"person farmer\" and\nplayer = \"player 1\"\n]\n]"
"player 2" 1.0 0 -1184463 true "" "if ticks > 0 [\nplot item 0[onfarm_inc] of turtles with [\nshape = \"person farmer\" and\nplayer = \"player 2\"\n]\n]"
"player 3" 1.0 0 -13840069 true "" "if ticks > 0 [\nplot item 0[onfarm_inc] of turtles with [\nshape = \"person farmer\" and\nplayer = \"player 3\"\n]\n]"
"player 4" 1.0 0 -955883 true "" "if ticks > 0 [\nplot item 0[onfarm_inc] of turtles with [\nshape = \"person farmer\" and\nplayer = \"player 4\"\n]\n]"
...
send_biomass
send_biomass
\"residue\" \"manure\" \"grain\" \"cattle\" \"small ruminant\" \"donkey\" \"poultry\" \"money\"
0
...
buy_what
buy_what
\"residue\" \"manure\" \"grain\" \"cattle\" \"small ruminant\" \"donkey\" \"poultry\" \"fertilizer\" \"transhumance\"
0
...
sell_what
sell_what
\"residue\" \"manure\" \"grain\" \"cattle\" \"small ruminant\" \"donkey\" \"poultry\" \"fertilizer\"
0
